export default function CaseStudies() {
    ...
    return (
        <section id="case-studies" className="py-32 relative">
            {/* Decorative Orbs */}
            <div className="absolute top-40 right-10 w-[300px] h-[300px] bg-purple-500/5 blur-[100px] rounded-full pointer-events-none -z-10 hardware-accelerated"></div>

            <div className="container mx-auto px-6">
                <div className="mb-20 text-center max-w-3xl mx-auto">
                    <div className="inline-block px-3 py-1 rounded-full border border-white/10 bg-white/5 text-emerald-400 font-semibold text-xs mb-4 tracking-wider uppercase">
                        Proven Impact
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    {cases.map((study, idx) => (
                        <div
                            key={idx}
                            className="group flex flex-col bg-slate-900/40 rounded-3xl p-10 border border-white/10 hover:border-emerald-500/30 transition-all duration-500 relative overflow-hidden shadow-2xl hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.1)] hardware-accelerated"
                        >
                            {/* Card internal glow on hover */}
                            <div className="absolute -top-32 -right-32 w-64 h-64 bg-emerald-500/15 blur-[50px] rounded-full group-hover:bg-emerald-500/25 transition-all duration-700 pointer-events-none hardware-accelerated"></div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-auto">
                                {study.tags.map((tag) => (
                                    <span
                                        key={tag}
                                        className="px-3 py-1.5 bg-white/5 text-slate-300 text-xs font-semibold rounded-lg border border-white/5 group-hover:border-white/10 transition-colors"
                                    >
                                        {tag}
                                    </span>
                                ))}
